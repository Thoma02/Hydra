import facebook from "../assets/icons/facebook.svg";
import twitter from "../assets/icons/twitter.svg";
import linkedin from "../assets/icons/linkedin.svg";
import youtube from "../assets/icons/youtube.svg";
import instagram from "../assets/icons/instagram.svg";
import pinterest from "../assets/icons/pinterest.svg";
import unreal from "../assets/icons/unreal.svg";
import unity from "../assets/icons/unity.svg";
import oculus from "../assets/icons/oculus.svg";
import vive from "../assets/icons/vive.svg";


export const navSiteLinks = [
    { url: "about", label: "ABOUT" },
    { url: "services", label: "SERVICES" },
    { url: "tech", label: "TECHNOLOGIES" },
    { url: "process", label: "HOW TO" },
];

export const techCompaniesLinks = [
    { url: "https://www.unrealengine.com/en-US", img: unreal, label: "Unreal Engine" },
    { url: "https://unity.com/", img: unity, label: "Unity" },
    { url: "https://www.oculus.com/experiences/quest/", img: oculus, label: "Oculus" },
    { url: "https://www.vive.com/de/", img: vive, label: "Vive" },
];

export const footerSiteLinks1 = [
    {url: "about", label: "ABOUT"},
    {url: "services", label: "SERVICES"},
    {url: "tech", label: "TECHNOLOGIES"},
    {url: "process", label: "HOW TO"},
    {url: "", label: "JOIN HYDRA"},
];

export const footerSiteLinks2 = [
        {url: "", label: "F.A.Q"},
        {url: "", label: "SITEMAP"},
        {url: "", label: "CONDITIONS"},
        {url: "", label: "LICENSES"},
];

export const footerMediaLinks = [
    {url: "https://facebook.com", label: facebook},
    {url: "https://twitter.com/", label: twitter},
    {url: "https://www.linkedin.com/", label: linkedin},
    {url: "https://www.youtube.com/", label: youtube},
    {url: "https://www.instagram.com/", label: instagram},
    {url: "https://www.pinterest.de/", label: pinterest},
];